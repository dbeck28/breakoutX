    ...
    func createDatabase() -> Bool {
        var created = false
            ...
            if database != nil {
                // Open the database.
                if database.open() {
                    let createUsersTable = "create table users (\(field_Users_UserID) integer primary key autoincrement not null, \(field_Users_UserName) text not null, \(field_Users_Email) text not null, \(field_Users_Password) text not null)"
